...
GOM_fnc_addAircraftLoadoutToObject = {
	...
	_action = ["<t color='#00FFFF'><img image='client\icons\repair.paa'/> Edit Aircraft Loadout</t>",{params ["_caller"];[_caller] spawn GOM_fnc_aircraftLoadout},[],0,true,true,"","_this isEqualTo vehicle _this AND {speed _this < 3} AND {alive _this} AND {alive _target}",3];
...
GOM_fnc_updateDialog = {
	...
	_fueltext = if (GOM_fnc_aircraftLoadout_NeedsFuelSource AND !_canRefuel) then {"<t color='#E51B1B'>Use the Resupply Trucks to refuel the aircraft!</t>"} else {""}; //Modified to only allow armament modifications in Wasteland
	_repairtext = if (GOM_fnc_aircraftLoadout_NeedsRepairSource AND !_canRepair) then {"<t color='#E51B1B'>(Use the Resupply Trucks to refuel and repair the aircraft!)</t>"} else {""};
	_rearmtext = if (GOM_fnc_aircraftLoadout_NeedsAmmoSource AND !_canRearm) then {"<t color='#E51B1B'>You need a new ammo source to rearm the aircraft!</t>"} else {""};
...
_getText = (_repairVehs call BIS_fnc_consolidateArray) apply {_t = (_t + " " + str (_x select 1) + " " + (getText (configfile >> "CfgVehicles" >> (_x select 0) >> "displayName")) + ",")};
	...
	_getText = (_rearmVehs call BIS_fnc_consolidateArray) apply {_t = (_t + " " + str (_x select 1) + " " + (getText (configfile >> "CfgVehicles" >> (_x select 0) >> "displayName")) + ",")};
		...
		_rearmtext = format ["<t color='#1BE521'>%1%2 ammo %3 nearby:%4</t>",_ammoInfo,count _rearmVehs,_s,_t select [0,((count _t) -1)]];

	};


	_text = format ["<t shadow='2' size='0.75'><t align='center'>Loadout Modification: %1<br /><br /><t align='center'>%2<br />%3",(_availableTexts select _canRearm),_rearmtext,_repairtext]; //Modified to only allow armament modifications in Wasteland
	_text = format ["<t shadow='2' size='0.75'><t align='center'>Repairing: %1<br />Refueling: %2<br />Rearming: %3<br /><br /><t align='center'>%4<br />%5<br />%6",(_availableTexts select _canRepair),(_availableTexts select _canRefuel),(_availableTexts select _canRearm),_repairtext,_fueltext,_rearmtext];
...
GOM_fnc_titleText = {
	...
	_date = format ["%1, %2.%3.%4",_weekday,_month,_day,_year];//suck it americans...NOT
...
//Cost of changing loadout
//*******************************************************
if (((getPos player) inArea mainAirfield1) || ((getPos player) inArea mainAirfield2) || ((getPos player) inArea saltlake1) || ((getPos player) inArea saltlake2) || ((getPos player) inArea selekano1) || ((getPos player) inArea selekano2)) then
{
PRICE = 100000;

			_title  = "<t color='#ff0000' size='1.2' align='center'>WARNING! </t><br />";
			_text1 = "<t color='#FFFFFF' align='center'>This service is currently EXPERIMENTAL and may not have satisfactory results.<br/><br/>Best results occur if the player exits to the Lobby and immediately returns BEFORE pressing OK to CONFIRM.</t><br /><br />";     
			_text2  = "<t color='#ff0000' size='1.2' align='center'>USE AT YOUR OWN RISK!</t><br /><br />";
			_text3 = "<t color='#FFFFFF' align='center'>The cost is $100000 to modify aircraft armament.<br/><br/>Press OK to confirm and be charged $100000.</t><br />";
			_message = parsetext (_title + _text1 + _text2 + _text3); 

if !([_message, "Confirm", true, true] call BIS_fnc_guiMessage) then { breakOut "GOM_fnc_aircraftLoadout" }; // cancel GOM loadout in area;
...
_money = player getVariable ["bmoney", 0];
...
//Deduct
player setVariable ["bmoney", _money - PRICE, true];
//*********************************************************
};
