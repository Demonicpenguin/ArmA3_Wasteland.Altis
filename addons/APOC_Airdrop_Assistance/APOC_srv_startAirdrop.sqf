...
switch (_type) do {
	...
	case "Armoured": 	{_selectionArray = APOC_AA_LightOptions};
	case "Tanks": 		{_selectionArray = APOC_AA_TankOptions};
	case "Helicoptors": {_selectionArray = APOC_AA_HeliOptions};	
	case "Boats": 		{_selectionArray = APOC_AA_BoatOptions};		
	case "supply": 		{_selectionArray = APOC_AA_SupOptions};
	//case "picnic":	{_selectionArray = APOC_AA_SupOptions};
	//case "Base":		{_selectionArray = APOC_AA_BaseOptions};
	case "base1":		{_selectionArray = APOC_AA_BaseOptions};
	case "base2":		{_selectionArray = APOC_AA_BaseOptions};	
...
_object = switch (_type) do {
	...
	case "Helicoptors":
	{
		...
		_object setVariable ["R3F_LOG_Disabled", true, true]; // Make vehicle locked
		_object lock 2; // Make vehicle locked
		[_object, true] call vehicleSetup;
		//[_object, "#(rgb,1,1,1)color(0.01,0.01,0.01,1)"] call applyVehicleTexture; // Apply black color
		if (_object getVariable ["A3W_purchasedVehicle", false] && !isNil "fn_manualVehicleSave") then
		{
			_object call fn_manualVehicleSave;
	...
	case "supply":
	{
		...
		If (_object isKindOf ST_ATCONVENIENCEKIT || _object isKindOf ST_ATCONVENIENCEKIT) then
		{
			//0 = [_object] execVM "server\missions\factoryMethods\STCreateConvenienceKit.sqf";
			[[_object], 'server\missions\factoryMethods\STCreateConvenienceKit.sqf'] remoteExec ['BIS_fnc_execVM', 0];			
		};	
...
WaitUntil {(((position _object) select 2) < (_flyHeight-20))};
		...
		_flare1= "F_40mm_Blue" createVehicle getPos _object;
...
/***************************************************************************************/		
	...
	sleep 0.3;
	_object setPos [(position _object) select 0, (position _object) select 1, 1];
	_object SetVelocity [0,0,0];
	_object allowDamage true; //Turn on damage for vehicles once they're in the 'chute.  Could move this until they hit the ground.  Admins choice.	
/***************************************************************************************/	
		
		_smoke2= "SmokeShellBlue" createVehicle getPos _object;
		//_smoke2 attachto [_object,[0,0,-0.5]];
		_flare2= "F_40mm_Blue" createVehicle getPos _object;
