...
Monkey_AirSupport_coolDownTime = 3600;	// Monkey Air Support Cool Down Timer
Monkey_AirSupport_coolDownTime2 = 1800; // Air Refueling Cool Down TImer 
BoS_coolDownTimer = 10800;           	// Baselocker hacking timer
Safe_coolDownTimer = 3600;          	// Safe hacking timer
...
A3W_disableTIforAll = 0; 		// Disable Thermal Vision for Everything
A3W_disableTIforLaunchers = 0;  // Disable Thermal Vision when using Launchers
A3W_disableTIforDrones = 1;     // Disable Thermal Vision when using Drones
A3W_disablevehicleThermals = 0;	// Disable Thermal Vision for Vehicles
...
// Napalm settings
//A3W_napalmSanctioned = 0;			// Admin allow Napalm bombs to be dropped (0 = No, 1 = Yes)
...
A3W_maxSpawnBeacons = 3;
A3W_maxSpawnBeaconsInv = 3;
...
A3W_payrollInterval2 = 1*60;       // Delay in seconds between each payroll
A3W_payrollAmount = 2000;           // Amount of money rewarded per territory on each payroll
A3W_payrollAmount2 = 2000;           // Amount of money rewarded per territory on each payroll
