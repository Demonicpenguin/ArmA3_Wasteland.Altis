...
_setupVars =
	{
		...
		_missionType = "Bounty Hunt - Catch the Rabbit";
...
_setupObjects =
{
	...
	_missionHintText = format ["<t align='center' color='%2' shadow='2' size='1.75'>Bounty Hunt</t><br/><t align='center' color='%2'>------------------------------</t><br/><t color='%3' size='1.0'>%1 on %4 has a bounty on his head. You have 30 minutes to catch the rabbit and kill them! Killer gets $500,000 and his side gets $50,000 per person. If he's protected he gets the $50,000 and his side gets $5,000 per person.</t>", name _foundPlayer, bountyMissionColor, subTextColor, _playerSideName];
	...
	//[format ["addBounty:%1:%2:%3:%4:%5:%6:%7:%8", call A3W_extDB_ServerID, call A3W_extDB_MapID, 'SYSTEM', getPlayerUID _foundPlayer, '0', _mission_state, 50000, 500000]] call extDB_Database_async;
...
_failedExec =
{
	...
	if (_mission_state == BOUNTY_MISSION_END_SURVIVED) then {
		...
		_playerMoney = _playerMoney + 500000;
	...
	if (_mission_state == BOUNTY_MISSION_END_AI) then {
		_failedHintMessage =  format ["%1, the Bounty Target, has survived!, earning $500k. %2 players will receive $50k for their support (Indy only if in group).", _playerName, _playerSide];
...
_successExec =
{
	...
	_successHintMessage = format ["%1, the Bounty Target, has been killed by %2, earning $500k. %3 players will receive $50k for their support (Indy only if in group).", _playerName, bKillerName, _killerSideName];
