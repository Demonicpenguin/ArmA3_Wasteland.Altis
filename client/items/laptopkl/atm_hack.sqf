...
null = [player] call compile preprocessFileLineNumbers "client\items\laptopkl\nearestTown.sqf";
...
_checks =
{
	...
	[_failed, _text];
	deleteMarker "atmHackLoc1"; 
	deleteMarker "atmHackLoc2";
};
...
_accounts =
{
	...
	_hackedTotalNumber = 0;
	{
		...
		if (isPlayer _hacked) then
		{
			...
			{
				_hackingPercentage = 0.01; // default of 0.01% 1 percent (being an INDY/GUER is harder to obtain money via capture zones).
				switch (true) do
				{
					case (side _hacked isEqualTo BLUFOR): {_hackingPercentage = 0.01}; // 1 percent (increase percentage based upon faction). less armour on uniforms.
					case (side _hacked isEqualTo OPFOR): {_hackingPercentage = 0.015}; // 1.5 percent (increase percentage based upon faction). more armour on uniforms.
				};
