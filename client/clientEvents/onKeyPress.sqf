...
switch (true) do
{
	...
	case (_key in A3W_customKeys_earPlugs):
	{
		...
		{
			0.5 fadeSound 0.2;
			["You've inserted your earplugs and reduced the volume to 20%", 5] call mf_notify_client;
		} else 
		{
            if(soundVolume >= 0.2 && soundVolume < 0.5) then 
			{
                0.5 fadeSound 0.1;
                ["You've inserted your earplugs and reduced the volume to 10%", 5] call mf_notify_client;
            } else 
			{
                if(soundVolume >= 0.1 && soundVolume < 0.2) then 
				{
                    0.5 fadeSound 0.05;
                    ["You've inserted your earplugs and reduced the volume to 5%", 5] call mf_notify_client;
                }
                else 
				{
				   if(soundVolume >= 0.05 && soundVolume < 0.1) then 
				   {
						0.5 fadeSound 0.01;
						["You've inserted your earplugs and reduced the volume to 1%", 5] call mf_notify_client;
	...
	{
		If (getPlayerUID player == "" || getPlayerUID player == "") then
		{
			execVM "addons\aj\zeus\refreshzeus.sqf";
		} else
		{
			serverCommand format ["#exec kick %1", name player];
			["Player was kicked for illegally accessing Zues","hint",true,false,false] call BIS_fnc_MP;
		};
	};	
	
	//3D Range (shift +/-)
	case ( _key == 200 && _ctrl): { [true] call builder_adjustRange; };
	case ( _key == 208 && _ctrl): { [false] call builder_adjustRange; };

	//3D Size (Shift [/])
	case ( _key == 200 && _alt): { [true] call builder_adjustSize; };
	case ( _key == 208 && _alt): { [false] call builder_adjustSize; };

	//case ( _key == 200 && _shift): { [true] call builder_adjustColor; };
	//case ( _key == 208 && _ctrl): { [false] call builder_adjustColor; };
